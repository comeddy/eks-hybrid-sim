...
> EKS 클러스터: ajt-simulator-cluster (v1.35)  
> 도메인: ajt-space.com
...
{
    "UserId": "AIDAXJRHSVQJP4X3VNWDR",
    "Account": "501520444434",
    "Arn": "arn:aws:iam::501520444434:user/admin"
}
```
...
{
    "clusters": ["ajt-simulator-cluster"]
}

$ aws eks describe-cluster --name ajt-simulator-cluster --region ap-northeast-2 \
    --query 'cluster.{Status:status,Endpoint:endpoint,Version:version}'
{
    "Status": "ACTIVE",
    "Endpoint": "https://B5BAC3551767E46C1DA1D66CE30FEAD9.gr7.ap-northeast-2.eks.amazonaws.com",
    "Version": "1.35"
}
```
...
[
    {
        "Name": "ajt-space.com.",
        "Id": "/hostedzone/Z06950993QD19U0IDPWIO"
    }
]
```
...
```bash
$ aws eks update-kubeconfig --name ajt-simulator-cluster --region ap-northeast-2
Updated context arn:aws:eks:ap-northeast-2:501520444434:cluster/ajt-simulator-cluster in /home/ec2-user/.kube/config

$ kubectl get nodes
NAME                                               STATUS   ROLES    AGE     VERSION
ip-172-31-29-174.ap-northeast-2.compute.internal   Ready    <none>   5d15h   v1.35.3-eks-bbe087e
ip-172-31-5-148.ap-northeast-2.compute.internal    Ready    <none>   5d15h   v1.35.3-eks-bbe087e
```
...
$ OIDC_URL=$(aws eks describe-cluster --name ajt-simulator-cluster --region ap-northeast-2 \
    --query 'cluster.identity.oidc.issuer' --output text)
$ echo $OIDC_URL
https://oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9
...
$ echo $THUMBPRINT
b5ff9c677325645101f7c5b456c6bfdb3c037425
...
{
    "OpenIDConnectProviderArn": "arn:aws:iam::501520444434:oidc-provider/oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9"
}
```
    ...
    --policy-document file:///tmp/alb-iam-policy.json
{
    "Policy": {
        ...
        "Arn": "arn:aws:iam::501520444434:policy/AWSLoadBalancerControllerIAMPolicy"
    }
}
```
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "Federated": "arn:aws:iam::501520444434:oidc-provider/oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9"
      },
      "Action": "sts:AssumeRoleWithWebIdentity",
      "Condition": {
        "StringEquals": {
          "oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9:aud": "sts.amazonaws.com",
          "oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9:sub": "system:serviceaccount:kube-system:aws-load-balancer-controller"
        }
      }
    }
  ]
    ...
    --assume-role-policy-document file:///tmp/alb-trust-policy.json
{
    "Role": {
        ...
        "Arn": "arn:aws:iam::501520444434:role/AmazonEKSLoadBalancerControllerRole"
    }
}

$ aws iam attach-role-policy \
    --role-name AmazonEKSLoadBalancerControllerRole \
    --policy-arn arn:aws:iam::501520444434:policy/AWSLoadBalancerControllerIAMPolicy
```
...
metadata:
  ...
  annotations:
    eks.amazonaws.com/role-arn: arn:aws:iam::501520444434:role/AmazonEKSLoadBalancerControllerRole
EOF
...
$ VPC_ID=$(aws eks describe-cluster --name ajt-simulator-cluster --region ap-northeast-2 \
    --query 'cluster.resourcesVpcConfig.vpcId' --output text)

$ helm install aws-load-balancer-controller eks/aws-load-balancer-controller \
    -n kube-system \
    --set clusterName=ajt-simulator-cluster \
...
501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/simulator-can
501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/simulator-server
501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/simulator-vehicle
501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/target-android
501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/target-cluster
```
...
base_domain      = "ajt-space.com"
eks_cluster_name = "ajt-simulator-cluster"
ecr_registry     = "501520444434.dkr.ecr.ap-northeast-2.amazonaws.com"
...
Changes to Outputs:
  ...
  + user_endpoints = {
      + "hyundai/user-a" = "https://user-a.hyundai.ajt-space.com"
      + "hyundai/user-b" = "https://user-b.hyundai.ajt-space.com"
      + "kia/user-a"     = "https://user-a.kia.ajt-space.com"
    }
```
...
| # | 리소스 | 설명 |
|---|--------|------|
| 1 | `module.acm["hyundai"].aws_acm_certificate.wildcard` | `*.hyundai.ajt-space.com` 인증서 |
| 2 | `module.acm["hyundai"].aws_route53_record.cert_validation` | ACM DNS 검증 CNAME |
| 3 | `module.acm["kia"].aws_acm_certificate.wildcard` | `*.kia.ajt-space.com` 인증서 |
...
acm_certificate_arns = {
  "hyundai" = "arn:aws:acm:ap-northeast-2:501520444434:certificate/3b2e3b17-68ad-49fc-b1ea-0e93105ab746"
  "kia"     = "arn:aws:acm:ap-northeast-2:501520444434:certificate/62137112-ba7d-48a9-a249-2d3cfd477622"
}
user_endpoints = {
  "hyundai/user-a" = "https://user-a.hyundai.ajt-space.com"
  "hyundai/user-b" = "https://user-b.hyundai.ajt-space.com"
...
sim-hyundai-user-a   hyundai-user-a   alb     user-a.hyundai.ajt-space.com   80      5d15h
sim-hyundai-user-b   hyundai-user-b   alb     user-b.hyundai.ajt-space.com   80      5d15h
sim-kia-user-a       kia-user-a       alb     user-a.kia.ajt-space.com       80      5d15h
...
$ aws acm describe-certificate \
    --certificate-arn arn:aws:acm:ap-northeast-2:501520444434:certificate/3b2e3b17-68ad-49fc-b1ea-0e93105ab746 \
    --query 'Certificate.Status'
"PENDING_VALIDATION"
```

**원인**: 도메인 `ajt-space.com`의 NS 레코드가 Route 53 네임서버로 위임되지 않았습니다.
...
$ aws ecr get-login-password --region ap-northeast-2 \
    | docker login --username AWS --password-stdin 501520444434.dkr.ecr.ap-northeast-2.amazonaws.com

# 각 컴포넌트별 이미지 Push (예시)
$ docker tag simulator-server:latest 501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/simulator-server:latest
$ docker push 501520444434.dkr.ecr.ap-northeast-2.amazonaws.com/simulator-server:latest
...
$ curl -k https://user-a.hyundai.ajt-space.com/health
```
...
| 리소스 | 이름/ARN | 용도 |
|--------|----------|------|
| OIDC Provider | `arn:aws:iam::501520444434:oidc-provider/oidc.eks.ap-northeast-2.amazonaws.com/id/B5BAC3551767E46C1DA1D66CE30FEAD9` | IRSA |
| IAM Policy | `arn:aws:iam::501520444434:policy/AWSLoadBalancerControllerIAMPolicy` | ALB Controller 권한 |
| IAM Role | `arn:aws:iam::501520444434:role/AmazonEKSLoadBalancerControllerRole` | ALB Controller ServiceAccount |
| ACM 인증서 | `arn:aws:acm:ap-northeast-2:501520444434:certificate/3b2e3b17-...` | `*.hyundai.ajt-space.com` |
| ACM 인증서 | `arn:aws:acm:ap-northeast-2:501520444434:certificate/62137112-...` | `*.kia.ajt-space.com` |
